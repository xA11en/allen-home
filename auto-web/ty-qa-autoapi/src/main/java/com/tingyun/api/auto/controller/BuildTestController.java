	...
	public String startTest(ModelMap modelMap,HttpServletRequest request) {
	    ...
	    try {
	    	 ...
	    	 for (int i = 86; i < list.size(); i++) 
	    	 {
				 ...
				 //数据存储的结果
				 String json = list.get(86).getJson();
	    		...
	    		{	
	    			...
	    			modelMap.addAttribute("caseName", list.get(i).getCaseName());
	    			modelMap.addAttribute(Constant.REPORT_FAIL_STATUS,"FAIL");
	    			modelMap.addAttribute("requestTou", AppHttpClient.convertCurlCMD(authKey, par, url));
	    		}	
			 }
		} catch (Exception e) {
